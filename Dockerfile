...
FROM notcompsky/wangle-static:latest AS intermediate
...
ENV CXXFLAGS="-static"
WORKDIR /rscraper
COPY tagger /rscraper/tagger
COPY shared-src /rscraper/shared-src
...
FROM alpine:latest
COPY --from=intermediate /rscraper/build/server /rscraper-server
EXPOSE 80
