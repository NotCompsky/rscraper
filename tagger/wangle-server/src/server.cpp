					...
					case '/':
						switch(*(s++)){
							case 'c':
								switch(*(s++)){
									case '/':
										return _r::comments_given_reason(s);
									default: return not_found;
								}
							case 'r':
								switch(*(s++)){
									case '/':
										return _r::subreddits_given_reason(s);
									default: return not_found;
								}
							default: return not_found;
						}
					...
					case '/':
						switch(*(s++)){
							case 'r':
								switch(*(s++)){
									case '/':
										return _r::subreddits_given_userid(s);
									default: return not_found;
								}
							default: return not_found;
						}
...
class RTaggerHandler : public wangle::HandlerAdapter<const char*,  const std::string_view> {
	public:
		void read(Context* ctx,  const char* msg) override {
			...
			char* itr = buf;
			const std::string_view r = determine_response(msg);
			while(*msg != '\n'  &&  *msg != 0  &&  buf_sz != 0){
				*(itr++) = *(msg++);
				--buf_sz;
			}
