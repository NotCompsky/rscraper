...
extern char* ITR;
extern char BUF[81000]; // Global large buffer
...
void populate_reason2name(){
	compsky::mysql::query_buffer(
		...
		"SELECT "
			"r.id,"
			"r.name,"
			"IFNULL(w.subreddit,0),"
			"IFNULL(b.subreddit,0) "
		"FROM reason_matched r "
		"LEFT JOIN reason_subreddit_blacklist b ON r.id=b.reason "
		"LEFT JOIN reason_subreddit_whitelist w ON r.id=w.reason "
		"ORDER BY r.id DESC"
	);
	int reason_id;
	char* name;
	uint64_t subreddit_wl, subreddit_bl;
	if(compsky::mysql::assign_next_row(_mysql::res1, &_mysql::row1, &reason_id, &name, &subreddit_wl, &subreddit_bl)){
	...
	} while(compsky::mysql::assign_next_row(_mysql::res1, &_mysql::row1, &reason_id, &name, &subreddit_wl, &subreddit_bl));
}
