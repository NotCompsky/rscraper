...
static const QRegularExpression highlighting_regex(
	"(?<![^\\\\]\\\\)(?:"
		...
		"(?<=^)([ \t]*)|"		// Whitespace after newline that is ignored by the hub's pre-processor (#5) // NOTE: Fails to not comment out lines preceded by an escape character - newlines work weirdly. // TODO: Fix this.
		"(\\$\\{[^}]+\\})"		// Variable substitution (#6) (not implemented into regex pre-processor yet, but planned)
	")|"
	"((?:^|[ \t]+)#.*)|"			// Comment (#7)
	"(\\\\[\\\\nrtv])"			// Escape sequence parsed by the hub's pre-processor (#8) // TODO: maybe ignore if preceded by an odd number of escape characters
);
    ...
    : QSyntaxHighlighter(parent)
{
    ...
    highlighting_fmts[2].setForeground(Qt::darkBlue);	// Capture group name (outer)
    highlighting_fmts[3].setForeground(Qt::darkBlue);	// Capture group name (inner)
    highlighting_fmts[3].setFontWeight(QFont::Bold);	// Capture group name (inner)
    ...
    highlighting_fmts[7].setBackground(cl_comment);	// Comment
    highlighting_fmts[8].setForeground(Qt::red);	// Escape characters parsed by pre-processor
    highlighting_fmts[5].setBackground(cl_comment);	// Comment
    highlighting_fmts[6].setForeground(Qt::yellow);	// Variable substitution
    highlighting_fmts[6].setBackground(cl_varsub);	// Variable substitution
    highlighting_fmts[6].setFontWeight(QFont::Bold);	// Variable substitution
