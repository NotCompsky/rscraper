...
void reason(QWidget* parent,  const QString& s){
	QMessageBox::information(parent, "Reason Not Found", "No reason named `" + s + "` was found in the database.\nHint: Use `rscrape-cmnts` and/or `rscraper-import reason_matched.csv` to populate the table - for instance, rscrape-cmnts populates the reason_matched table based on the comment body regex (found in the 'longstrings' table and edited in the hub's __SCRAPER__ tab).\nIf you are sure the entry exists, restart this program so that the caches are updated.");
}

void user(QWidget* parent,  const QString& s){
	QMessageBox::information(parent, "Reason Not Found", "No user named `" + s + "` was found in the database.\nHint: Use `rscrape-cmnts` and/or `rscraper-import user.csv` to populate the table - for instance, rscrape-cmnts populates the user table based on which comments match the regex in the comment body regexp (found in the 'longstrings' table and edited in the hub's __SCRAPER__ tab).\nIf you are sure the entry exists, restart this program so that the caches are updated.");
}
